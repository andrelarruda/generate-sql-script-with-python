...
try:
    ...
    result_temp = "select [Plan] as 'Plano', count(Id) as 'Quantidade de instâncias' from cash.global.ApplicationInstances where SubDomain='" + "' or SubDomain = '".join(dataframe['Instância'])
    ...
    # result_temp = "select Id, SubDomain, [Plan] from cash.global.ApplicationInstances where (SubDomain='" + "' or SubDomain = '".join(dataframe['Instância'])
